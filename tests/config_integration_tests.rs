...
fn test_production_config_service() {
    ...
    match ProductionConfigService::with_env_provider(std::sync::Arc::new(env)) {
        ...
        Err(_) => {
            // In test environment there might not be a config file, which is normal
        }
    }
}
...
async fn test_config_with_file_manager() {
    // Use new test tools for file management tests
...
fn test_multiple_config_instances() {
    ...
    // Ensure config instances are isolated from each other
...
fn test_config_builder_chaining() {
    // Test chained calls on config builder
    let config = TestConfigBuilder::new()
        .with_ai_provider("openai")
        .with_ai_model("gpt-4.1")
        .build_config();
