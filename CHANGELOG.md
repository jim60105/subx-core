...
### Added
- OpenSpec root: the twelve core halves of the split mixed capabilities — `cache-management`, `component-factory`, `configuration-management`, `encoding-detection`, `error-handling`, `format-conversion`, `input-path-handling`, `parallel-processing`, `secrets-protection`, `subtitle-matching`, `subtitle-translation`, `timeline-sync` — 91 requirements imported via `openspec/changes/import-split-capability-specs/` (archived same day). Each arriving requirement holds only the clauses testable against this repository's tree; `subx-cli` obligations are named in prose as belonging to the same-named capability in that repository (spec-governance bidirectional drift record). The `local-llm-provider` spec's three `subx-cli`-qualified citations of `configuration-management`/`error-handling` re-qualify to this repository. Spec-only: no code, test, API or dependency change.
- OpenSpec root: `openspec init` plus the thirteen core capabilities imported from [`subx-cli`](https://github.com/jim60105/subx-cli) — 96 requirements / 254 scenarios reproduced verbatim (see `openspec/changes/archive/2026-09-06-import-core-specs/`), their `openspec/specs/` file history carried by an unrelated-histories merge of `subx-cli`'s filtered spec history (the merge resolves the add/add on `openspec/specs/**` to ours, so the filtered donor commits are reachable only through the merge's second parent: read them with `git log --full-history -- openspec/specs/…` or `git log --follow`, not the default history simplification), and thirteen hand-written `## Purpose` paragraphs (twelve carried, `core-reporting`'s new). Spec-only: no code, test, API or dependency change.
...
- Handoff record from the C2a import, for the capabilities' next editors: `local-llm-provider`'s three references to `configuration-management`/`error-handling` are qualified "in `subx-cli`" and become `subx-core`'s when proposal C2b splits those capabilities; `subtitle-parser-hardening`'s `fuzz/` and `scripts/quality_check.sh` citations are root-relative to this repository, per `spec-governance`'s own-repo citation rule, with `subx-cli/Cargo.toml` kept qualified as the other repository's file; and the `spec-governance` capability (which stays in `subx-cli`) is the rule set every future move is checked against. The drift check `spec-governance` specifies is not implemented here; C3 gives it a home in the quality scripts, and `subx-core/AGENTS.md` (not yet existing) inherits the two-root rules C3 authors.
...
### Documentation
- `AGENTS.md` rewritten for the standalone library crate — library-only project overview with both consumers, crate-specific module guide including the `report` seam, the one-way dependency-boundary prohibition, core's own quality-script contract, the `test-support` testing layout, and the independent `openspec/` root — carrying a shared-conventions region byte-identical with `subx-cli/AGENTS.md`, enforced by the superproject's new `--check-spec-governance` drift check.
- `README.md` rewritten for crates.io/standalone consumers: the two consumers, the supported standalone-clone workflow, the `archive-rar`/`slow-tests`/`test-support` feature flags, the caret relationship to `subx-cli`'s dependency line, core's own workflow badge (no crates.io/docs.rs badges until the first release exists to badge), and absolute-URL links into the superproject's docs.
- `[package.metadata.docs.rs]` narrowed from `all-features` to `features = ["archive-rar"]`, so the documentation-site build will not activate `test-support` when the crate is first published — `subx_core::test_support::*` will not be presented as documented public API and `wiremock`/`hound` stay out of the documentation build.
