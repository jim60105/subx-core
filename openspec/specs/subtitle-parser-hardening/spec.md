...
tests MUST be gated behind the existing `slow-tests` cargo feature —
`subx-core`'s own declaration, the real gate over the tests that live here — so the
...
`Cargo.lock` resolution is unchanged regardless of feature flags, read
against `subx-core`'s own committed lockfile).
