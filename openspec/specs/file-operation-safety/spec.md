...
Eliminate TOCTOU races, silent overwrites, and symlink-escape
vulnerabilities in the file operations performed during match and
relocation. Every file-creating path — copy, move/rename, and backup —
must claim its destination atomically and validate the parent directory
chain against the expected output directory.
