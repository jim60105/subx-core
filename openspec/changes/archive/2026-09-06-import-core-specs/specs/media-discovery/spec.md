...
`FileDiscovery::scan_directory(root, recursive)` SHALL descend into subdirectories when `recursive = true` and SHALL limit traversal to the root directory only (`max_depth(1)`) when `recursive = false`. The recursion flag originates from the CLI `--recursive` option as consumed by `subx-cli`'s match command.
